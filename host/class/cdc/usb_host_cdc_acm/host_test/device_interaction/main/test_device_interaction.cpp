...
static void _add_mocked_devices(void)
{
    ...
    REQUIRE(ESP_OK == usb_host_mock_add_device(1, (const usb_device_desc_t *)premium_cord_device_desc_fs,
            (const usb_config_desc_t *)premium_cord_config_desc_fs));

    // ASIX Electronics Corp. AX88772B (FS descriptor)
    REQUIRE(ESP_OK == usb_host_mock_add_device(2, (const usb_device_desc_t *)i_tec_device_desc_fs,
            (const usb_config_desc_t *)i_tec_config_desc_fs));

    // FTDI chip dual (FS descriptor)
    REQUIRE(ESP_OK == usb_host_mock_add_device(3, (const usb_device_desc_t *)ftdi_device_desc_fs_hs,
            (const usb_config_desc_t *)ftdi_config_desc_fs));

    // TTL232RG (FS descriptor)
    REQUIRE(ESP_OK == usb_host_mock_add_device(4, (const usb_device_desc_t *)ttl232_device_desc,
...
static void _submit_mock_transfer(cdc_acm_dev_hdl_t *dev)
{
    ...
    REQUIRE(ESP_OK == mock_cdc_acm_host_data_tx_blocking(*dev, tx_buf, sizeof(tx_buf), 200, MOCK_USB_TRANSFER_SUCCESS));
    // Submit transfer which will fail to submit
    REQUIRE(ESP_ERR_INVALID_RESPONSE == mock_cdc_acm_host_data_tx_blocking(*dev, tx_buf, sizeof(tx_buf), 200, MOCK_USB_TRANSFER_ERROR));
...
SCENARIO("Interact with mocked USB devices")
{
    ...
    GIVEN("Mocked devices are added to the device list") {
        ...
        SECTION("Interact with device: ASIX Electronics Corp. AX88772A Fast Ethernet") {
            ...
            // Open a device
            REQUIRE(ESP_OK == mock_cdc_acm_host_open(device_address, vid, pid, interface_index, &dev_config, &dev));
            REQUIRE(dev != nullptr);
            // Interact with the device - submit mocked transfers
            _submit_mock_transfer(&dev);
            // Close the device
            REQUIRE(ESP_OK == mock_cdc_acm_host_close(&dev, interface_index));
        }
