...
/**
 * @brief Test: remote wakeup enable/disable
 *
 * #. open/suspend the device
 * #. submit a non-ctrl transfer, expect the device to resume
 * #. submit a ctrl transfer, expect the device to resume
 * #. cleanup
 */
TEST_CASE("remote_wake_enable_disable", "[remote_wake]")
{
    nb_of_responses = 0;
    TEST_ASSERT_NOT_NULL(app_queue = xQueueCreate(5, sizeof(cdc_acm_host_dev_event_data_t)));
    ...
    bool enabled = false;
    printf("Check\n");
    TEST_ASSERT_EQUAL(ESP_OK, cdc_acm_host_remote_wake_check(cdc_dev, &enabled));
    TEST_ASSERT_FALSE(enabled);

    printf("Enable\n");
    TEST_ASSERT_EQUAL(ESP_OK, cdc_acm_host_remote_wake_enable(cdc_dev, true));
    ...
    printf("Disable\n");
    TEST_ASSERT_EQUAL(ESP_OK, cdc_acm_host_remote_wake_enable(cdc_dev, false));
...
TEST_CASE("remote_wake_trigger", "[remote_wake]")
{
    nb_of_responses = 0;
    TEST_ASSERT_NOT_NULL(app_queue = xQueueCreate(5, sizeof(cdc_acm_host_dev_event_data_t)));
