...
bool frame_callback(const uvc_frame_t *frame, void *user_ctx)
{
    assert(frame);
    assert(user_ctx);
...
static void frame_handling_task(void *arg)
{
    ...
    QueueHandle_t frame_q = *((QueueHandle_t *)(stream_config->user_arg));
...
void app_main(void)
{
    ...
    for (int i = 0; i < EXAMPLE_NUMBER_OF_STREAMS; i++) {
        rx_frames_queue[i] = xQueueCreate(EXAMPLE_FRAME_COUNT, sizeof(uvc_frame_t *));
    ...
    BaseType_t task_created = xTaskCreatePinnedToCore(usb_lib_task, "usb_lib", 4096, NULL, EXAMPLE_USB_HOST_PRIORITY, NULL, 0);
