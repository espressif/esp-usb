...
static void uvc_known_devices_free_all(uvc_host_driver_t *uvc_obj)
{
    assert(uvc_obj);
...
static bool uvc_known_devices_remove_all(uint8_t dev_addr, bool notify)
{
...
#ifdef USB_HOST_HAS_DEV_REMOVED_EVENT
    case USB_HOST_CLIENT_EVENT_DEV_REMOVED:
        ESP_LOGD(TAG, "Device removed, addr: %d", event_msg->dev_removed.address);
        if (!uvc_known_devices_remove_all(event_msg->dev_removed.address, true)) {
...
esp_err_t uvc_host_uninstall()
{
    ...
    uvc_known_devices_free_all(uvc_obj);
