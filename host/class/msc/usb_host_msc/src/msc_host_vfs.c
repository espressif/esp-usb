...
#ifdef MSC_HOST_BDL_API_SUPPORTED
/* IDF's diskio_bdl.c owns pdrv assignment and (un)registration, keyed by `bdl`;
 * no need to cache drive/pdrv here. */
typedef struct msc_host_vfs {
    char *base_path;
    esp_blockdev_handle_t bdl; /* borrowed from msc_device_t; not released here */
} msc_host_vfs_t;
#else
/* This component owns diskio registration (ff_diskio_register_msc), so drive/pdrv
...
#ifdef MSC_HOST_BDL_API_SUPPORTED
    /* pdrv isn't cached in vfs_handle (see struct above), so resolve it here on demand;
     * format() is rare enough that this lookup cost doesn't matter. */
