...
/*
These tests validate the HCD's per-transfer size-limit enforcement for all transfer types. The limit is the maximum
number of bytes the controller can move in a single transfer; what bounds it depends on the transfer type (see the
table below):
    - Bulk/control: the 17-bit non-isochronous qTD "Total bytes to transfer" field (Scatter/Gather DMA,
      programming guide Section 6)
    - Interrupt/isochronous: the descriptor list length, at one qTD per packet
...
TEST_CASE("Test HCD control transfer size limit", "[ctrl][full_speed][high_speed]")
{
    ...
    const int limit = hcd_pipe_get_xfer_size_limit(default_pipe);
...
TEST_CASE("Test HCD bulk transfer size limit", "[bulk][full_speed][high_speed]")
{
    ...
    const int in_limit = hcd_pipe_get_xfer_size_limit(bulk_in_pipe);
    const int out_limit = hcd_pipe_get_xfer_size_limit(bulk_out_pipe);
...
TEST_CASE("Test HCD interrupt transfer size limit", "[intr][full_speed][high_speed]")
{
    ...
    // Halt each pipe immediately after enqueue: at HS (bInterval=1) a transfer to this non-existent
    // address hits XCS_XACT (3 transaction timeouts) after ~3 microframes and would report
    // HCD_PIPE_EVENT_ERROR_XFER instead of URB_DONE if the halt is delayed
