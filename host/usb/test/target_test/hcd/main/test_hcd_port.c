...
TEST_CASE("Test HCD port remote wakeup", "[port][low_speed][full_speed][high_speed][ignore]")
{
    ...
    printf("Do the remote wakeup (Keyboard button press)\n");
    ...
    TEST_ASSERT_EQUAL_MESSAGE(false, test_hcd_remote_wake_check(default_pipe, get_status_urb), "Remote wake-up is disabled, but expected to be enabled");
