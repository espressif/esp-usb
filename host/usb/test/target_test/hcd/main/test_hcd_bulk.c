...
TEST_CASE("Test HCD bulk pipe URBs ZLP", "[bulk][full_speed][high_speed]")
{
    ...
    ESP_LOGI(TAG, "Data OUT + ZLP terminal event: %d, status: %d, actual_num_bytes: %d",
    ...
    if (data_event == HCD_PIPE_EVENT_ERROR_STALL) {
        ...
        ESP_LOGI(TAG, "Device STALLed the host-issued ZLP (expected for strict MSC devices); recovering pipe");
        ...
        TEST_HCD_EXPECT_TRANSFER_STATUS(urb_data_wr, USB_TRANSFER_STATUS_COMPLETED);
    }
