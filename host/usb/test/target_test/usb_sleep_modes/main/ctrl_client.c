...
static void ctrl_transfer_cb(usb_transfer_t *transfer)
{
    ...
    // CTRL transfer is done, signalize light sleep task to enter light sleep
...
void ctrl_client_task(void *arg)
{
    ...
    ctrl_obj.test_event_group = (EventGroupHandle_t)arg;
    ...
    while (!exit_loop) {
        ...
        case TEST_STAGE_CTRL_XFER: {
            ...
            USB_SETUP_PACKET_INIT_GET_CONFIG_DESC((usb_setup_packet_t *)ctrl_xfer->data_buffer, 0, MAX_TRANSFER_BYTES);
            ctrl_xfer->num_bytes = sizeof(usb_setup_packet_t) + MAX_TRANSFER_BYTES;
            ctrl_xfer->bEndpointAddress = 0x80;
