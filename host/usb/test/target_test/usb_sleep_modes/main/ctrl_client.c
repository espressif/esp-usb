...
typedef struct {
    ...
    usb_host_client_event_t last_event;             /**< Device's last event */
...
static void ctrl_client_event_cb(const usb_host_client_event_msg_t *event_msg, void *arg)
{
    ...
    case USB_HOST_CLIENT_EVENT_DEV_RESUMED:
        ...
        // Make sure deferred suspend event is delivered before the resume event
        TEST_ASSERT_EQUAL_MESSAGE(ctrl_obj->last_event, USB_HOST_CLIENT_EVENT_DEV_SUSPENDED, "Suspend event not delivered");
        // Only relevant to light sleep test, check if the device is manually resumed after the light sleep and schedule next stage accordingly
    ...
    ctrl_obj->last_event = event_msg->event;
}
