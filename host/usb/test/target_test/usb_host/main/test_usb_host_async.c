...
- Test can be run with either a device which has or which does not have remote wakeup feature
    - this is controlled in menuconfig with CONFIG_USB_HOST_TEST_DEVICE_HAS_REMOTE_WAKEUP
...
Procedure:
    ...
    - For the device which does not have the feature:
        - Try to check the feature is enabled and try to enable it (2 separate public API calls)
    - For the device which does have the feature:
        - Check the feature (must be disabled) -> Enable the feature -> Check the feature (must be enabled)
        - Suspend the root port and expect the suspend event
        - Manually induce the remote wakeup (button press on mouse or keyboard) and expect resume event
        - Check the feature (must still be enabled) -> Disable the feature -> Check the feature (must be disabled)
    - Teardown
*/
#define TEST_REMOTE_WAKE_SMP_WAIT_MS 2000
#ifdef CONFIG_USB_HOST_TEST_DEVICE_HAS_REMOTE_WAKEUP
#define HAS_REMOTE_WAKE 1
#else
#define HAS_REMOTE_WAKE 0
#endif
...
TEST_CASE("Test USB Host remote wakeup setup", "[usb_host][low_speed][full_speed][high_speed]")
{
    ...
    // Or to return ESP_ERR_NOT_ALLOWED if the device does not have the feature
    test_remote_wake_check(false, HAS_REMOTE_WAKE);
    TEST_ASSERT_EQUAL_MESSAGE(
        pdTRUE, xSemaphoreTake(dev_ready_smp, TEST_REMOTE_WAKE_SMP_WAIT_MS), "Remote wake not checked on time");

    // Enable remote wakeup on the device which has the feature,
    // Or expect an error ESP_ERR_NOT_ALLOWED if the device does not have the feature
    test_remote_wake_enable(HAS_REMOTE_WAKE);
    TEST_ASSERT_EQUAL_MESSAGE(
        pdTRUE, xSemaphoreTake(dev_ready_smp, TEST_REMOTE_WAKE_SMP_WAIT_MS), "Remote wake not enabled on time");

    // Continue with further checks only for the devices with remote wakeup enabled
    if (HAS_REMOTE_WAKE) {

        // Read the current remote wakeup state from the device, expect it to be enabled from the previous operation
        test_remote_wake_check(true, HAS_REMOTE_WAKE);
        TEST_ASSERT_EQUAL_MESSAGE(
            pdTRUE, xSemaphoreTake(dev_ready_smp, TEST_REMOTE_WAKE_SMP_WAIT_MS), "Remote wake not checked on time");
        ...
        TEST_ASSERT_EQUAL(ESP_OK, usb_host_lib_root_port_suspend());
        TEST_ASSERT_EQUAL_MESSAGE(
            pdTRUE, xSemaphoreTake(dev_ready_smp, TEST_REMOTE_WAKE_SMP_WAIT_MS), "Suspend event not generated on time");

        // Do remote wake
        printf("Expecting remote wake event\n");
        TEST_ASSERT_EQUAL_MESSAGE(
            pdTRUE, xSemaphoreTake(dev_ready_smp, TEST_REMOTE_WAKE_SMP_WAIT_MS * 2), "Resume event not generated on time");
        ...
        test_remote_wake_disable(HAS_REMOTE_WAKE);
        TEST_ASSERT_EQUAL_MESSAGE(
