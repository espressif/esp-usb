...
Procedure:
    ...
    - If the device supports the remote wakeup, test enabling, disabling and reading the current value of the remote wakeup
    - Teardown
*/
TEST_CASE("Test USB Host remote wakeup", "[usb_host][low_speed][full_speed][high_speed]")
{
    TaskHandle_t client_task = NULL;
    xTaskCreate(ctrl_client_remote_wake_task, "remote_wake client", 4096, NULL, 2, &client_task);
    TEST_ASSERT_NOT_NULL_MESSAGE(client_task, "Failed to create async client task");
    // Start the task
    xTaskNotifyGive(client_task);
