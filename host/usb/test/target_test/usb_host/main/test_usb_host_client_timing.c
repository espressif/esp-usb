...
/*
 * Keep the opt-in cases under a local macro so this file can be bisected with older host APIs if needed.
 */
#define USB_HOST_TEST_ENUM_EXISTING_SUPPORTED 1
...
#if USB_HOST_TEST_ENUM_EXISTING_SUPPORTED
static esp_err_t register_timing_client_with_existing_enum_queue_size(client_timing_ctx_t *ctx, usb_host_client_handle_t *client_hdl, int max_num_event_msg)
{
    const usb_host_client_config_t client_config = {
        .is_synchronous = false,
        .max_num_event_msg = max_num_event_msg,
        .flags = {
            .enumerate_existing_devices = 1,
        },
        .async = {
            .client_event_callback = client_timing_cb,
            .callback_arg = ctx,
    ...
    return usb_host_client_register(&client_config, client_hdl);
}

static void register_timing_client_with_existing_enum(client_timing_ctx_t *ctx, usb_host_client_handle_t *client_hdl)
{
    TEST_ASSERT_EQUAL(ESP_OK, register_timing_client_with_existing_enum_queue_size(ctx, client_hdl, 5));
}
#endif
...
TEST_CASE("USB Host clients registered after enumeration do not receive historical NEW_DEV by default", "[usb_host][timing][low_speed][full_speed][high_speed]")
{
    client_timing_ctx_t ctx[2] = {};
    usb_host_client_handle_t client_hdl[2] = {};
    ...
    TEST_ASSERT_EQUAL(0, client0_new_dev_count);
    TEST_ASSERT_EQUAL(0, client1_new_dev_count);
}
...
TEST_CASE("USB Host mixed early and late clients preserve per-client NEW_DEV timing", "[usb_host][timing][low_speed][full_speed][high_speed]")
{
    ...
    TEST_ASSERT_EQUAL(0, late_client_new_dev_count);
}
...
TEST_CASE("USB Host late clients receive NEW_DEV for later reconnect", "[usb_host][timing][low_speed][full_speed][high_speed]")
{
    ...
    TEST_ASSERT_EQUAL(0, client0_historical_count);
    TEST_ASSERT_EQUAL(0, client1_historical_count);
}

TEST_CASE("USB Host multiple late clients do not cross-deliver historical NEW_DEV by default", "[usb_host][timing][low_speed][full_speed][high_speed]")
{
    ...
    for (int i = 0; i < TEST_CLIENT_TIMING_MAX_CLIENTS; i++) {
        TEST_ASSERT_EQUAL(0, new_dev_count[i]);
    }
}
...
#if USB_HOST_TEST_ENUM_EXISTING_SUPPORTED
TEST_CASE("USB Host opt-in late client receives existing device as NEW_DEV", "[usb_host][timing][low_speed][full_speed][high_speed]")
{
    client_timing_ctx_t ctx = {};
    usb_host_client_handle_t client_hdl = NULL;

    wait_for_existing_device_ready();
    register_timing_client_with_existing_enum(&ctx, &client_hdl);
    wait_for_new_dev_events(&client_hdl, &ctx, 1, 1);
...
TEST_CASE("USB Host opt-in late client receives one NEW_DEV across pending enumeration boundary", "[usb_host][timing][low_speed][full_speed][high_speed]")
{
    client_timing_ctx_t ctx = {};
    usb_host_client_handle_t client_hdl = NULL;
...
TEST_CASE("USB Host opt-in client does not receive NEW_DEV after all devices are freed", "[usb_host][timing][low_speed][full_speed][high_speed]")
{
    client_timing_ctx_t ctx = {};
    usb_host_client_handle_t client_hdl = NULL;
...
TEST_CASE("USB Host opt-in registration tolerates device removal in flight", "[usb_host][timing][low_speed][full_speed][high_speed]")
{
    client_timing_ctx_t ctx = {};
    usb_host_client_handle_t client_hdl = NULL;
...
TEST_CASE("USB Host opt-in late clients and early clients share consistent NEW_DEV visibility", "[usb_host][timing][low_speed][full_speed][high_speed]")
{
    client_timing_ctx_t ctx[3] = {};
    usb_host_client_handle_t client_hdl[3] = {};

    power_off_root_and_wait_empty();
    register_timing_client_with_existing_enum(&ctx[0], &client_hdl[0]);
    power_on_root_and_wait_one_device();
    wait_for_new_dev_events(client_hdl, ctx, 1, 1);
    drain_events_for(client_hdl, 1, TEST_CLIENT_TIMING_NO_EVENT_MS);

    register_timing_client_with_existing_enum(&ctx[1], &client_hdl[1]);
    register_timing_client_with_existing_enum(&ctx[2], &client_hdl[2]);
