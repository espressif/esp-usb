 ...
 *
 * @param[in] has_remote_wake: Specify if the connected device features with the remote wakeup functionality
 */
 ...
 * @note This function must not be called from the same task as the client task, as a control transfer is sent
 *
 * @param[in] expected_current_remote_wake: Expected current state of the remote wakeup feature on the device
 * @param[in] has_remote_wake: Specify if the connected device features with the remote wakeup functionality
 */
