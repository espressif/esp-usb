...
struct client_s {
    ...
    struct {
        ...
        dev_addr_map_t pending_existing_dev_addr_map;
    } mux_protected;
...
static inline bool dev_addr_map_test(dev_addr_map_t dev_addr_map, uint8_t dev_addr)
{
    assert(dev_addr <= 127);
...
static void send_event_msg_to_clients(const usb_host_client_event_msg_t *event_msg, bool send_to_all, uint8_t opened_dev_addr)
{
    ...
    TAILQ_FOREACH(client_obj, &p_host_lib_obj->mux_protected.client_tailq, dynamic.tailq_entry) {
        if (!send_to_all) {
            ...
            HOST_ENTER_CRITICAL();
            bool send = dev_addr_map_test(client_obj->dynamic.opened_dev_addr_map, opened_dev_addr);
            HOST_EXIT_CRITICAL();
            if (!send) {
                continue;
            }
...
static void send_new_dev_event_msg_to_clients(uint8_t dev_addr)
{
    ...
    // Mark the device visible while holding mux_lock so later client registration cannot miss this device.
    dev_addr_map_set(p_host_lib_obj->mux_protected.client_visible_dev_addr_map, dev_addr);
    xSemaphoreGive(p_host_lib_obj->constant.mux_lock);
}

static void send_pending_existing_new_dev_events_to_client(client_t *client_obj)
{
    ...
    for (uint8_t dev_addr = 1; dev_addr <= 127; dev_addr++) {
        if (!dev_addr_map_test(client_obj->mux_protected.pending_existing_dev_addr_map, dev_addr)) {
            continue;
        }
        const usb_host_client_event_msg_t event_msg = {
            .event = USB_HOST_CLIENT_EVENT_NEW_DEV,
        ...
        dev_addr_map_clear(client_obj->mux_protected.pending_existing_dev_addr_map, dev_addr);
    }
}
...
static void clear_all_client_visible_devices(void)
{
    ...
    client_t *client_obj;
    TAILQ_FOREACH(client_obj, &p_host_lib_obj->mux_protected.client_tailq, dynamic.tailq_entry) {
        dev_addr_map_clear_all(client_obj->mux_protected.pending_existing_dev_addr_map);
    }
    xSemaphoreGive(p_host_lib_obj->constant.mux_lock);
}
...
static void send_removed_event_msg_to_clients(uint8_t dev_addr)
{
    ...
    TAILQ_FOREACH(client_obj, &p_host_lib_obj->mux_protected.client_tailq, dynamic.tailq_entry) {
        ...
        HOST_ENTER_CRITICAL();
        bool opened = dev_addr_map_test(client_obj->dynamic.opened_dev_addr_map, dev_addr);
        HOST_EXIT_CRITICAL();
        if (opened) {
            continue;
        }
...
static void usbh_event_callback(usbh_event_data_t *event_data, void *arg)
{
    ...
    case USBH_EVENT_DEV_GONE: {
        if (event_data->dev_gone_data.dev_addr == 0) {
            ...
            ESP_LOGD(USB_HOST_TAG, "Skip client DEV_GONE for default address device");
            break;
        }
    ...
    case USBH_EVENT_DEV_REMOVED:
        if (event_data->dev_removed_data.dev_addr == 0) {
            // Address 0 is used only during early enumeration and is never visible to host clients.
            ESP_LOGD(USB_HOST_TAG, "Skip client DEV_REMOVED for default address device");
            break;
        }
...
esp_err_t usb_host_client_register(const usb_host_client_config_t *client_config, usb_host_client_handle_t *client_hdl_ret)
{
    ...
    if (client_config->flags.enumerate_existing_devices) {
        dev_addr_map_copy(client_obj->mux_protected.pending_existing_dev_addr_map, p_host_lib_obj->mux_protected.client_visible_dev_addr_map);
        send_pending_existing_new_dev_events_to_client(client_obj);
    }
    xSemaphoreGive(p_host_lib_obj->constant.mux_lock);
...
esp_err_t usb_host_device_open(usb_host_client_handle_t client_hdl, uint8_t dev_addr, usb_device_handle_t *dev_hdl_ret)
{
    ...
    HOST_ENTER_CRITICAL();
    if (dev_addr_map_test(client_obj->dynamic.opened_dev_addr_map, dev_addr)) {
...
esp_err_t usb_host_interface_claim(usb_host_client_handle_t client_hdl, usb_device_handle_t dev_hdl, uint8_t bInterfaceNumber, uint8_t bAlternateSetting)
{
    ...
    HOST_CHECK_FROM_CRIT(dev_addr_map_test(client_obj->dynamic.opened_dev_addr_map, dev_addr), ESP_ERR_INVALID_STATE);
