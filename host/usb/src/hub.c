...
static esp_err_t dev_tree_node_new(void *parent, uint8_t port_num, usb_speed_t speed)
{
    ...
    if (parent == NULL) {
        /* TODO: Port index <-> port number */
        uint8_t port_idx = port_num;
        // uint8_t port_idx = port_num - 1;
        root_port_hdl = p_hub_driver_obj->constant.root_port_hdls[port_idx].constant.hdl;
    } else {
#if (ENABLE_USB_HUBS)
...
static bool root_port_callback(hcd_port_handle_t port_hdl, hcd_port_event_t port_event, void *user_arg, bool in_isr)
{
    ...
    // uint8_t port_idx = hcd_port_get_number(port_hdl) - 1;
...
static void root_port_handle_events(root_port_t *root_port)
{
    ...
    uint8_t port_num = hcd_port_get_number(root_port_hdl);
    ...
    case HCD_PORT_EVENT_CONNECTION: {
        ...
        if (dev_tree_node_new(NULL, port_num, speed) != ESP_OK) {
...
reset_err:
    ...
    case HCD_PORT_EVENT_OVERCURRENT: {
        ...
        if (port_has_device) {
            ESP_ERROR_CHECK(dev_tree_node_dev_gone(NULL, port_num));
        }
...
static esp_err_t root_port_recycle(uint8_t port_num)
{
    /* TODO: Port index <-> port number */
    uint8_t port_idx = port_num;
    // uint8_t port_idx = port_num - 1;
...
static esp_err_t root_port_reset(uint8_t port_num)
{
    /* TODO: Port index <-> port number */
    uint8_t port_idx = port_num;
    // uint8_t port_idx = port_num - 1;
    ...
    if (ret != ESP_OK) {
        ...
        ret = dev_tree_node_reset_completed(NULL, port_num);
    }
    return ret;
}

static esp_err_t root_port_disable(uint8_t port_num)
{
    /* TODO: Port index <-> port number */
    uint8_t port_idx = port_num;
    // uint8_t port_idx = port_num - 1;
    ...
    *client_ret = NULL;
    ...
    for (uint8_t port_idx = 0; port_idx < HUB_ROOT_PORTS; port_idx++) {
        ...
        if (hub_config->port_map & (1 << port_idx)) {
            /* TODO: Port index <-> port number */
            // port_idx = 0 -> USB OTG HS
            // port_idx = 1 -> USB OTG FS
...
#endif  // ENABLE_USB_HUBS
    info->dev_addr = parent_dev_addr;
    info->port_num = dev_tree_node->port_num;
    return ESP_OK;
}
...
static bool root_get_pending_port(uint8_t *port_num)
{
...
{
    uint8_t port_num;
    while (root_get_pending_port(&port_num)) {
        /* TODO: Port index <-> port number */
        uint8_t port_idx = port_num;
        // uint8_t port_idx = port_num - 1;
