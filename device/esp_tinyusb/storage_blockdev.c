...
static esp_blockdev_handle_t _bdl_handle = ESP_BLOCKDEV_HANDLE_INVALID; // Borrowed, not owned; one instance at a time (see singleton guard below)
static size_t _bdl_sector_size = 0; // Cached at open time; valid only while _bdl_handle is valid

/* Mirrors FatFS's FF_MIN_SS/FF_MAX_SS (ffconf.h), which depend on
 * CONFIG_WL_SECTOR_SIZE - hardcoding 4096 here would drift from FatFS's
 * real max sector size if that Kconfig is set to 512.
 * TODO: duplicates diskio_bdl.c's compute_fs_sector_size(); replace with
 * a public IDF helper once one is exposed. */
#define MSC_BLOCKDEV_MIN_SECTOR_SIZE  512
#define MSC_BLOCKDEV_MAX_SECTOR_SIZE  ((CONFIG_WL_SECTOR_SIZE > 512) ? CONFIG_WL_SECTOR_SIZE : 512)
 ...
 * clamped to [MSC_BLOCKDEV_MIN_SECTOR_SIZE, MSC_BLOCKDEV_MAX_SECTOR_SIZE],
 * i.e. FatFS's real [FF_MIN_SS, FF_MAX_SS] for the project's CONFIG_WL_SECTOR_SIZE).
 *
 * Pure function of the handle's geometry; called once at open time and
 * cached in _bdl_sector_size to avoid recomputing on every I/O transfer.
 *
 * @return valid power-of-two sector size, or 0 if geometry is incompatible.
 */
static size_t storage_blockdev_compute_sector_size(esp_blockdev_handle_t bdl_handle)
{
...
static esp_err_t storage_blockdev_get_info(storage_info_t *info)
{
    ...
    uint64_t total_sectors_64 = _bdl_handle->geometry.disk_size / (uint64_t)_bdl_sector_size;
    ESP_RETURN_ON_FALSE(total_sectors_64 <= UINT32_MAX, ESP_ERR_INVALID_SIZE, TAG,
                        ...
                        total_sectors_64, (unsigned)_bdl_sector_size);

    info->sector_size = (uint32_t)_bdl_sector_size;
...
static esp_err_t storage_blockdev_sector_read(uint32_t lba, uint32_t offset, size_t size, void *dest)
{
    ...
    uint64_t addr = (uint64_t)lba * _bdl_sector_size + offset;
...
esp_err_t storage_blockdev_open_medium(esp_blockdev_handle_t bdl_handle, const storage_medium_t **medium)
{
    ...
    size_t sector_size = storage_blockdev_compute_sector_size(bdl_handle);
    ESP_RETURN_ON_FALSE(sector_size > 0, ESP_ERR_NOT_SUPPORTED, TAG,
                        "Block device geometry incompatible (no valid sector size derivable)");

    _bdl_handle = bdl_handle;
    _bdl_sector_size = sector_size;
