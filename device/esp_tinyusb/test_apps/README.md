...
Check VID and PID of the device used during testing, to correctly set UDEV rules `{idVendor}` and `{idProduct}` attributes
...
- `--volumes /dev/bus/usb:/dev/bus/usb`: mount volumes: Share low level access to usb devices with the docker container, otherwise the container crates it's own, internal device tree
