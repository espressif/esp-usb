...
TEST_CASE("Config: Default macros arguments", "[runtime_config][default]")
{
    ...
    // const tinyusb_config_t tusb_cfg_arg3 = TINYUSB_DEFAULT_CONFIG(dummy_event_hdl, dummy_event_arg, NULL); // Enable to verify static assert during the build

    TEST_ASSERT_EQUAL_MESSAGE(NULL, tusb_cfg_arg0.event_cb, "Event callback should be NULL when no argument are provided");
    TEST_ASSERT_EQUAL_MESSAGE(NULL, tusb_cfg_arg0.event_arg, "Event argument should be NULL when no no argument provided");
