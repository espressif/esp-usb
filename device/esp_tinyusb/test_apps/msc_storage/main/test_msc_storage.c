 ...
 */
TEST_CASE("MSC storage: SPIFLASH", "[storage][spiflash]")
{
    ...
    TEST_ASSERT_EQUAL(ESP_OK, tinyusb_msc_storage_new_spiflash(&config));
...
TEST_CASE("MSC storage: SDMMC", "[storage][sdmmc]")
{
    ...
    // Initialize TinyUSB MSC storage
    TEST_ASSERT_EQUAL(ESP_OK, tinyusb_msc_storage_new_mmc(&config));
    // Wait for the storage to be mounted
    test_storage_wait_callback(TINYUSB_MSC_EVENT_MOUNT_START);
    test_storage_wait_callback(TINYUSB_MSC_EVENT_MOUNT_COMPLETE);
    ...
    TEST_ASSERT_EQUAL_MESSAGE(card->csd.capacity, tinyusb_msc_storage_get_sector_count(), "SDMMC card capacity does not match TinyUSB MSC storage sector count");
    TEST_ASSERT_EQUAL_MESSAGE(card->csd.sector_size, tinyusb_msc_storage_get_sector_size(), "SDMMC card sector size does not match TinyUSB MSC storage sector size");
 ...
 */
TEST_CASE("MSC storage: specific base path", "[storage][spiflash]")
{
    _test_storage_event_queue = xQueueCreate(TEST_QUEUE_LEN, sizeof(test_storage_event_t));
    TEST_ASSERT_NOT_NULL(_test_storage_event_queue);
 ...
 * 7. Delete the storage event queue.
 */
TEST_CASE("MSC storage: do not format", "[storage][spiflash]")
{
    _test_storage_event_queue = xQueueCreate(TEST_QUEUE_LEN, sizeof(test_storage_event_t));
    TEST_ASSERT_NOT_NULL(_test_storage_event_queue);
    ...
    // Initialize TinyUSB MSC storage
    TEST_ASSERT_EQUAL(ESP_ERR_NOT_FOUND, tinyusb_msc_storage_new_spiflash(&config));
    test_storage_wait_callback(TINYUSB_MSC_EVENT_MOUNT_START);
