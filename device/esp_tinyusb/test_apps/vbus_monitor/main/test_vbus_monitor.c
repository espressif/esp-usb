...
#else
// For all other supported targets, use the *_IN_IDX variant
...
#if (TEST_BVALID_VIA_SIGNAL_MUX_ENABLED)
#if (CONFIG_IDF_TARGET_ESP32P4)
    printf("\t - might be triggered by signal multiplexing (only for USB OTG 1.1).\n");
#else
    printf("\t - might be triggered by signal multiplexing.\n");
#endif // CONFIG_IDF_TARGET_ESP32P4
#endif // TEST_BVALID_VIA_SIGNAL_MUX_ENABLED

    // Driving the GOTGCTL register directly is for all targets with DWC OTG controller, but we
    // verify it only for USB OTG 2.0
    // Note:
...
static uint8_t test_vbus_manual_attach_detach(void)
{
    ...
    while (rounds--) {
        printf("[ACTION REQUIRED] Detach device from the Host port ...\n");
        // Allow some time for the operator to remove the device
        vTaskDelay(pdMS_TO_TICKS(DEVICE_DETACH_ROUND_DELAY_MS));
        // Wait for the TINYUSB_EVENT_DETACHED event
        test_device_wait_event(TINYUSB_EVENT_DETACHED);
        printf("[       EVENT OK]\n");
        ...
        // Allow some time for the operator to connect the device
        vTaskDelay(pdMS_TO_TICKS(DEVICE_DETACH_ROUND_DELAY_MS));
        // Wait for the TINYUSB_EVENT_ATTACHED event
        test_device_wait_event(TINYUSB_EVENT_ATTACHED);
        printf("[       EVENT OK]\n");
...
#if ((SOC_USB_OTG_PERIPH_NUM == 1) && (TEST_BVALID_VIA_SIGNAL_MUX_ENABLED == 1))
...
#endif // ((SOC_USB_OTG_PERIPH_NUM == 1) && (TEST_BVALID_VIA_SIGNAL_MUX_ENABLED == 1))
