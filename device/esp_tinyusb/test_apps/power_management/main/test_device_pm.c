...
#define DATA_RECEPTION_WAIT_MS                  5000
...
TEST_CASE("tinyusb_remote_wakeup_reporting", "[esp_tinyusb][device_pm_remote_wake]")
{
    ...
    // pytest never opened tty device, just uses pyusb to interact with the device thus Linux host cdc-acm driver does not automatically enable remote wakeup
...
TEST_CASE("tinyusb_usb_wakeup_api", "[esp_tinyusb][device_esp_sleep][device_pm]")
{
    ...
    // Call esp_sleep for HS port capable targets
