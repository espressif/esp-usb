...
/**
 * @brief User defined tinyusb suspend callback
 */
void tud_suspend_cb(bool remote_wakeup_en)
{
    ESP_LOGI(TAG, "User defined suspend callback triggered");
    if (remote_wakeup_en) {
        printf("USER_EVENT_SUSPENDED_REMOTE_WAKE_EN\n");
    } else {
        printf("USER_EVENT_SUSPENDED_REMOTE_WAKE_DIS\n");
    }

    tinyusb_pm_notify_suspended();
    test_pm_notify_user_suspend();
}
#endif // !CONFIG_TINYUSB_SUSPEND_CALLBACK

#if !CONFIG_TINYUSB_RESUME_CALLBACK
/**
 * @brief User defined tinyusb resume callback
 */
void tud_resume_cb(void)
{
    ESP_LOGI(TAG, "User defined resume callback");
    printf("USER_EVENT_RESUMED\n");

    tinyusb_pm_notify_resumed();
    test_pm_notify_user_resume();
}
#endif // !CONFIG_TINYUSB_RESUME_CALLBACK

#if CONFIG_TINYUSB_SUSPEND_CALLBACK && CONFIG_TINYUSB_RESUME_CALLBACK && !CONFIG_TINYUSB_USB_OTG_WAKEUP
...
#endif // CONFIG_TINYUSB_SUSPEND_CALLBACK && CONFIG_TINYUSB_RESUME_CALLBACK && !CONFIG_TINYUSB_USB_OTG_WAKEUP
...
#else
    const uint32_t suspend_event = EVENT_BITS_USER_SUSPEND;
    const uint32_t resume_event = EVENT_BITS_USER_RESUME;
#endif
...
#endif // CONFIG_TINYUSB_SUSPEND_CALLBACK && CONFIG_TINYUSB_RESUME_CALLBACK

#if !CONFIG_TINYUSB_SUSPEND_CALLBACK && !CONFIG_TINYUSB_RESUME_CALLBACK

/**
 * @brief Verify PM lock via user-provided tud_suspend_cb/tud_resume_cb and notify APIs
 */
TEST_CASE("tinyusb_power_management_usb_wakeup_user_events", "[device_pm][tinyusb_pm_otg_wake_user_events]")
{
    const test_pm_install_opts_t opts = {
        .pm_lock_enable = true,
        .auto_light_sleep_enable = true,
    ...
    TEST_ASSERT_EQUAL(ESP_OK, esp_sleep_disable_usb_wakeup());
    TEST_ASSERT_EQUAL(ESP_OK, esp_sleep_cpu_retention_deinit());
    ESP_LOGI(TAG, "PM usb wakeup test user events cleanup");
}
#endif // !CONFIG_TINYUSB_SUSPEND_CALLBACK && !CONFIG_TINYUSB_RESUME_CALLBACK
