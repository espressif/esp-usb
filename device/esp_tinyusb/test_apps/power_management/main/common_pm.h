...
#define DATA_RECEPTION_WAIT_MS                  5000    /** Maximum time to wait for CDC RX data, in milliseconds. */
...
#define EVENT_BITS_USER_SUSPEND                 BIT4    /**< User-provided suspend callback notification */
#define EVENT_BITS_USER_RESUME                  BIT5    /**< User-provided resume callback notification */
#define EVENT_BITS_DETACHED                     BIT6    /**< Device detached event */
...
#define DEVICE_EVENT_BITS_ALL   (EVENT_BITS_ATTACHED |                  \
                                 ...
                                 EVENT_BITS_RESUMED |                   \
                                 EVENT_BITS_USER_SUSPEND |              \
                                 EVENT_BITS_USER_RESUME |               \
                                 EVENT_BITS_DETACHED)
...
/**
 * @brief Signal a user-provided suspend callback to waiting test code
 *
 * Sets `EVENT_BITS_USER_SUSPEND` in the PM test event group. Used from application
 * `tud_suspend_cb()` implementations when esp_tinyusb suspend callbacks are disabled.
 */
void test_pm_notify_user_suspend(void);

/**
 * @brief Signal a user-provided resume callback to waiting test code
 *
 * Sets `EVENT_BITS_USER_RESUME` in the PM test event group. Used from application
 * `tud_resume_cb()` implementations when esp_tinyusb resume callbacks are disabled.
 */
void test_pm_notify_user_resume(void);
