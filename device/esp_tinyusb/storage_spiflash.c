...
static uint32_t storage_spiflash_get_sector_count(wl_handle_t wl_handle)
{
    ...
    if (size == 0) {
        ...
        result = (uint32_t)(wl_size(wl_handle) / size);
    }
    return result;
}

static uint32_t storage_spiflash_get_sector_size(wl_handle_t wl_handle)
{
    assert(wl_handle != WL_INVALID_HANDLE);
    return (uint32_t)wl_sector_size(wl_handle);
}
        ...
        void *dest)
{
    ...
    uint32_t sector_size = storage_spiflash_get_sector_size(wl_handle);
...
esp_err_t storage_spiflash_init(void *ctx, const storage_api_t **storage_api)
{
    wl_handle_t wl_handle = (wl_handle_t)ctx;
