...
**Generic Block Device Storage (IDF >= 6.0.4)**
...
> **Note:** `tinyusb_msc_new_storage_spiflash()`/`_sdmmc()` remain available and unchanged — migration is optional for now. Prefer `tinyusb_msc_new_storage_blockdev()` for forward-compatibility with future `esp_blockdev`-backed media.
