...
// diskio_bdl.c (storage_blockdev.c's dependency) shipped in IDF 6.0.4/6.1.0, not 6.0.0-6.0.3.
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(6, 0, 4)
#define TINYUSB_MSC_BDL_SUPPORTED 1
#include "esp_blockdev.h"
#endif // ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(6, 0, 4)
...
typedef struct {
    union {
        ...
        esp_blockdev_handle_t blockdev;      /*!< Generic block device handle (IDF >= 6.0.4). */
 ...
 *
 * Recommended on ESP-IDF >= 6.0.4 for any medium exposed as an esp_blockdev
