...
static inline size_t storage_sdmmc_get_sector_count(sdmmc_card_t *card)
{
    assert(card);
    return (size_t)card->csd.capacity;
}

static inline size_t storage_sdmmc_get_sector_size(sdmmc_card_t *card)
{
    assert(card);
    assert(card->csd.sector_size != 0); // Sector size must be non-zero if SDMMC card was initialized correctly
    return (size_t)card->csd.sector_size;
}
...
static esp_err_t storage_sdmmc_get_info(void *ctx, storage_info_t *info)
{
    ...
    sdmmc_card_t *card = (sdmmc_card_t *)ctx;
    info->total_sectors = (uint32_t) storage_sdmmc_get_sector_count(card);
    info->sector_size = (uint32_t) storage_sdmmc_get_sector_size(card);
    return ESP_OK;
}
