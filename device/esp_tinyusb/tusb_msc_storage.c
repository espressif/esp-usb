...
typedef struct {
    ...
    volatile bool write_in_progress;        /*!< Indicates if a write operation is currently in progress.
                                                 This flag ensures that the filesystem is not mounted during an
                                                 ongoing write. It is marked as volatile because it can be modified
                                                 from different contexts, such as interrupt handlers or deferred
                                                 functions. */
...
static void _write_func(void *param)
{
    ...
    s_storage_handle->write_in_progress = false;
    if (err != ESP_OK) {
...
esp_err_t tinyusb_msc_storage_mount(const char *base_path)
{
    ...
    if (s_storage_handle->write_in_progress) {
