...
#define MSC_STORAGE_BUFFER_SIZE                                                \
  CONFIG_TINYUSB_MSC_BUFSIZE /*!< Size of the buffer, configured via           \
                                menuconfig (MSC FIFO size) */

#if ((MSC_STORAGE_BUFFER_SIZE) % MSC_STORAGE_MEM_ALIGN != 0)
#error                                                                         \
    "CONFIG_TINYUSB_MSC_BUFSIZE must be divisible by MSC_STORAGE_MEM_ALIGN. Adjust your configuration (MSC FIFO size) in menuconfig."
#endif
 ...
 */
typedef struct {
  uint8_t data_buffer[MSC_STORAGE_BUFFER_SIZE]; /*!< Buffer to store write data.
                                                   The size is defined by
                                                   MSC_STORAGE_BUFFER_SIZE. */
  uint32_t lba; /*!< Logical Block Address for the current WRITE10 operation. */
  uint32_t offset;  /*!< Offset within the specified LBA for the current write
                       operation. */
...
typedef struct {
  ...
  bool is_fat_mounted;   /*!< Indicates if the FAT filesystem is currently
                            mounted. */
  ...
  esp_err_t (*read)(
      size_t sector_size, /*!< Function pointer for reading data. */
      uint32_t lba, uint32_t offset, size_t size, void *dest);
  esp_err_t (*write)(
      size_t sector_size, /*!< Function pointer for writing data. */
      size_t addr, uint32_t lba, uint32_t offset, size_t size, const void *src);
  tusb_msc_callback_t
      callback_mount_changed; /*!< Callback for mount state change. */
  tusb_msc_callback_t
      callback_premount_changed; /*!< Callback for pre-mount state change. */
...
static esp_err_t _read_sector_spiflash(size_t sector_size, uint32_t lba,
                                       uint32_t offset, size_t size,
                                       void *dest) {
  size_t temp = 0;
  size_t addr = 0; // Address of the data to be read, relative to the beginning
                   // of the partition.
  ESP_RETURN_ON_FALSE(!__builtin_umul_overflow(lba, sector_size, &temp),
                      ESP_ERR_INVALID_SIZE, TAG,
                      "overflow lba %lu sector_size %u", lba, sector_size);
  ESP_RETURN_ON_FALSE(!__builtin_uadd_overflow(temp, offset, &addr),
                      ESP_ERR_INVALID_SIZE, TAG, "overflow addr %u offset %lu",
                      temp, offset);
  return wl_read(s_storage_handle->wl_handle, addr, dest, size);
}

static esp_err_t _write_sector_spiflash(size_t sector_size, size_t addr,
...
static esp_err_t _read_sector_sdmmc(size_t sector_size, uint32_t lba,
                                    uint32_t offset, size_t size, void *dest) {
  return sdmmc_read_sectors(s_storage_handle->card, dest, lba,
                            size / sector_size);
}

static esp_err_t _write_sector_sdmmc(size_t sector_size, size_t addr,
                                     uint32_t lba, uint32_t offset, size_t size,
                                     const void *src) {
  return sdmmc_write_sectors(s_storage_handle->card, src, lba,
                             size / sector_size);
}
#endif

static esp_err_t _msc_storage_read_sector(uint32_t lba, uint32_t offset,
...
static esp_err_t _msc_storage_write_sector(uint32_t lba, uint32_t offset,
                                           size_t size, const void *src) {
  assert(s_storage_handle);
  ...
  if (addr % sector_size != 0 || size % sector_size != 0) {
    ESP_LOGE(TAG,
             "Invalid Argument lba(%lu) offset(%lu) size(%u) sector_size(%u)",
             lba, offset, size, sector_size);
...
static esp_err_t _mount(char *drv, FATFS *fs) {
  ...
  if (fresult != FR_OK) {
    ...
    size_t alloc_unit_size =
        esp_vfs_fat_get_allocation_unit_size(CONFIG_WL_SECTOR_SIZE, 4096);
 ...
 *
 * This function is invoked via TinyUSB's deferred execution mechanism to
 * perform write operations to the underlying storage. It writes data from the
 * `storage_buffer` stored within the `s_storage_handle`.
 *
 * @param param Unused. Present for compatibility with deferred function
 * signature.
 */
...
esp_err_t tinyusb_msc_storage_mount(const char *base_path) {
  ...
  if (cb) {
    tinyusb_msc_event_t event = {
        ...
        .mount_changed_data = {.is_mounted = s_storage_handle->is_fat_mounted}};
    cb(&event);
  }
  ...
  ESP_GOTO_ON_ERROR((s_storage_handle->mount)(pdrv), fail, TAG,
                    "Failed pdrv=%d", pdrv);
...
esp_err_t
tinyusb_msc_storage_init_spiflash(const tinyusb_msc_spiflash_config_t *config) {
  assert(!s_storage_handle);
  ESP_RETURN_ON_FALSE(CONFIG_TINYUSB_MSC_BUFSIZE >= CONFIG_WL_SECTOR_SIZE,
                      ESP_ERR_NOT_SUPPORTED, TAG,
                      "CONFIG_TINYUSB_MSC_BUFSIZE (%d) must be at least the "
                      "size of CONFIG_WL_SECTOR_SIZE (%d)",
                      (int)(CONFIG_TINYUSB_MSC_BUFSIZE),
                      (int)(CONFIG_WL_SECTOR_SIZE));
  s_storage_handle = (tinyusb_msc_storage_handle_s *)heap_caps_aligned_alloc(
      MSC_STORAGE_MEM_ALIGN, sizeof(tinyusb_msc_storage_handle_s),
      MALLOC_CAP_DMA);
  ESP_RETURN_ON_FALSE(s_storage_handle, ESP_ERR_NO_MEM, TAG,
                      "Failed to allocate memory for storage handle");
...
#if SOC_SDMMC_HOST_SUPPORTED
esp_err_t
tinyusb_msc_storage_init_sdmmc(const tinyusb_msc_sdmmc_config_t *config) {
  assert(!s_storage_handle);
  s_storage_handle = (tinyusb_msc_storage_handle_s *)heap_caps_aligned_alloc(
      MSC_STORAGE_MEM_ALIGN, sizeof(tinyusb_msc_storage_handle_s),
      MALLOC_CAP_DMA);
...
#define SCSI_CODE_ASC_MEDIUM_NOT_PRESENT                                       \
  0x3A /** SCSI ASC code for 'MEDIUM NOT PRESENT' **/
#define SCSI_CODE_ASC_INVALID_COMMAND_OPERATION_CODE                           \
  0x20 /** SCSI ASC code for 'INVALID COMMAND OPERATION CODE' **/
...
bool tud_msc_test_unit_ready_cb(uint8_t lun) {
  ...
  if (s_storage_handle->is_fat_mounted) {
    tud_msc_set_sense(lun, SCSI_SENSE_NOT_READY,
                      SCSI_CODE_ASC_MEDIUM_NOT_PRESENT, SCSI_CODE_ASCQ);
    result = false;
  } else {
...
int32_t tud_msc_scsi_cb(uint8_t lun, uint8_t const scsi_cmd[16], void *buffer,
  ...
  default:
    ...
    tud_msc_set_sense(lun, SCSI_SENSE_ILLEGAL_REQUEST,
                      SCSI_CODE_ASC_INVALID_COMMAND_OPERATION_CODE,
                      SCSI_CODE_ASCQ);
    ret = -1;
    break;
  }
...
void tud_mount_cb(void) { tinyusb_msc_storage_unmount(); }
/*********************************************************************** TinyUSB
 * MSC callbacks*/
