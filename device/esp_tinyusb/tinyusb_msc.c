                                           ...
                                           tinyusb_msc_storage_handle_t *handle)
{
    ...
    // Sector size is only known after the medium is open. MSC FIFO must hold
    // at least one whole sector, or read10/write10 will fail on partial sectors.
    storage_info_t info;
    ...
    if (CONFIG_TINYUSB_MSC_BUFSIZE < info.sector_size) {
        ESP_LOGE(TAG, "TinyUSB buffer size (%d) must be at least the block device sector size (%" PRIu32 "), please reconfigure the project.",
