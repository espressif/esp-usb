...
typedef struct {
    ...
    const storage_api_t *base;                  /*!< Pointer to the storage API for the medium. */
...
{
    assert(p_msc_driver);
    void *context = p_msc_driver->dynamic.storage->context;
    tinyusb_msc_storage_s *s_storage_handle = p_msc_driver->dynamic.storage;
    return s_storage_handle->base->read(context, lba, offset, size, dest);
}
...
static inline esp_err_t msc_storage_write_sector(uint32_t lba, uint32_t offset, size_t size, const void *src)
{
    ...
    void *context = p_msc_driver->dynamic.storage->context;
    tinyusb_msc_storage_s *s_storage_handle = p_msc_driver->dynamic.storage;
    return s_storage_handle->base->write(context, lba, offset, size, src);
}
...
esp_err_t msc_storage_mount(void)
{
    ...
    ESP_GOTO_ON_ERROR(s_storage_handle->base->mount(s_storage_handle->context, pdrv), fail, TAG, "Failed pdrv=%d", pdrv);
...
static esp_err_t msc_storage_unmount(void)
{
    ...
    esp_err_t err = s_storage_handle->base->unmount(s_storage_handle->context);
    if (err) {
        return err;
    }
...
static esp_err_t msc_storage_config(msc_storage_obj_t *storage_obj, const tinyusb_msc_storage_config_t *config, const storage_api_t *storage_api)
{
    storage_obj->context = config->context;
    storage_obj->base = storage_api;
    ...
    ESP_RETURN_ON_ERROR(storage_obj->base->get_info(storage_obj->context, &storage_info), TAG, "Failed to get storage info");
...
esp_err_t tinyusb_msc_storage_set_callback(tusb_msc_callback_t callback, void *arg)
{
...
esp_err_t tinyusb_msc_driver_install(const tinyusb_msc_driver_config_t *config)
{
...
esp_err_t tinyusb_msc_driver_uninstall(void)
{
...
esp_err_t tinyusb_msc_storage_new_spiflash(const tinyusb_msc_storage_config_t *config,
        tinyusb_msc_storage_handle_t *handle)
{
    ...
    if (p_msc_driver == NULL) {
        ...
        ESP_RETURN_ON_ERROR(tinyusb_msc_driver_install(&default_cfg), TAG, "Failed to install MSC driver");
    ...
    ESP_RETURN_ON_ERROR(msc_storage_config(storage_obj, config, storage_api), TAG, "Failed to configure MSC storage");
...
#if (SOC_SDMMC_HOST_SUPPORTED)
esp_err_t tinyusb_msc_storage_new_mmc(const tinyusb_msc_storage_config_t *config,
                                      tinyusb_msc_storage_handle_t *handle)
{
    ...
    // Create a storage
    const storage_api_t *storage_api = NULL;
    ESP_RETURN_ON_ERROR(storage_sdmmc_get(config->context, &storage_api), TAG, "Failed to initialize MMC storage");
    ESP_RETURN_ON_ERROR(msc_storage_config(storage_obj, config, storage_api), TAG, "Failed to configure MSC storage");
    // Set the storage handle in the driver
    p_msc_driver->dynamic.storage = storage_obj;
    p_msc_driver->dynamic.lun_count++;
...
esp_err_t tinyusb_msc_storage_delete(tinyusb_msc_storage_handle_t handle)
{
    ...
    if (p_msc_driver->dynamic.lun_count == 0 && p_msc_driver->constant.flags.internally_installed) {
        ...
        ESP_ERROR_CHECK(tinyusb_msc_driver_uninstall()); // Should never fail
    }
    return ESP_OK;
}

esp_err_t tinyusb_msc_storage_get_sector_count(tinyusb_msc_storage_handle_t handle, uint32_t *sector_count)
{
...
esp_err_t tinyusb_msc_storage_get_sector_size(tinyusb_msc_storage_handle_t handle, uint32_t *sector_size)
{
    (void) handle; // Unused parameter, we use the global s_storage_handle
    ESP_RETURN_ON_FALSE(p_msc_driver != NULL, ESP_ERR_INVALID_STATE, TAG, "MSC driver is not initialized");
    ESP_RETURN_ON_FALSE(p_msc_driver->dynamic.storage != NULL, ESP_ERR_INVALID_STATE, TAG, "MSC storage is not initialized");
...
esp_err_t tinyusb_msc_storage_set_mount_point(tinyusb_msc_storage_handle_t handle,
...
esp_err_t tinyusb_msc_storage_get_mount_point(tinyusb_msc_storage_handle_t handle,
        tinyusb_msc_mount_point_t *mount_point)
{
    (void) handle; // Unused parameter, we use the global s_storage_handle
    ESP_RETURN_ON_FALSE(p_msc_driver != NULL, ESP_ERR_INVALID_STATE, TAG, "MSC driver is not initialized");
...
esp_err_t tinyusb_msc_storage_format_drive(tinyusb_msc_storage_handle_t handle)
{
...
void tud_msc_capacity_cb(uint8_t lun, uint32_t *block_count, uint16_t *block_size)
{
    ...
    tinyusb_msc_storage_get_sector_size(NULL, &sec_size);
    tinyusb_msc_storage_get_sector_count(NULL, &sec_count);
