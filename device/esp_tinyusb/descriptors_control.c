...
esp_err_t tinyusb_descriptors_check(tinyusb_port_t port, const tinyusb_desc_config_t *config)
{
    ...
    if (port == TINYUSB_PORT_1) {
#if (TUD_OPT_HIGH_SPEED)
        //
#else
        ESP_RETURN_ON_FALSE(port == TINYUSB_PORT_0, ESP_ERR_INVALID_ARG, TAG, "Only one port is available");
#endif // TUD_OPT_HIGH_SPEED
    }
...
esp_err_t tinyusb_descriptors_set(tinyusb_port_t port, const tinyusb_desc_config_t *config)
{
    ...
    if (port == TINYUSB_PORT_1) {
...
#else
            // Default configuration descriptor must be provided for the following classes
            ESP_GOTO_ON_FALSE(config->high_speed_config, ESP_ERR_INVALID_ARG, fail, TAG, "Full-speed configuration descriptor must be provided for this device");
#endif
        } else {
            s_desc_cfg.hs_cfg = config->high_speed_config;
        }

        // High-speed and Full-speed descriptors should be equal length
        ESP_GOTO_ON_FALSE(((tusb_desc_configuration_t *)s_desc_cfg.hs_cfg)->wTotalLength ==
                          ...
                          ESP_ERR_INVALID_ARG, fail, TAG, "High-speed and Full-speed configuration descriptors must be same length");
    ...
    if (config->string == NULL) {
        ...
        s_desc_cfg.str_count = (config->string_count != 0)
                               ? config->string_count
