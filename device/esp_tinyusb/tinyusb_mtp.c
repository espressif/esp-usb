...
// Tracks one SendPartialObject data phase; the edit session itself is held in active_edit.
typedef struct {
    bool active;
    uint32_t handle;
    uint64_t offset;
...
typedef struct {
    ...
    char *serial;
    char *friendly_name;
    struct tinyusb_mtp_storage_s storages[CONFIG_TINYUSB_MTP_MAX_STORAGES];
    mtp_object_t objects[CONFIG_TINYUSB_MTP_MAX_OBJECTS];
...
static int32_t mtp_set_object_prop_value(tud_mtp_cb_data_t *cb_data);
...
{
    free(s_mtp.manufacturer);
    free(s_mtp.model);
    free(s_mtp.version);
    free(s_mtp.serial);
    free(s_mtp.friendly_name);
    s_mtp.manufacturer = NULL;
    s_mtp.model = NULL;
    s_mtp.version = NULL;
    s_mtp.serial = NULL;
    s_mtp.friendly_name = NULL;
}

static void mtp_clear_pending_write(void)
{
    if (s_mtp.pending_write.file) {
        fclose(s_mtp.pending_write.file);
    }
    free(s_mtp.pending_write.path);
    free(s_mtp.pending_write.write_path);
    free(s_mtp.pending_write.backup_path);
    memset(&s_mtp.pending_write, 0, sizeof(s_mtp.pending_write));
}

static void mtp_clear_active_read(void)
{
    if (s_mtp.active_read.file) {
        fclose(s_mtp.active_read.file);
    }
    free(s_mtp.active_read.path);
    memset(&s_mtp.active_read, 0, sizeof(s_mtp.active_read));
}

static void mtp_clear_active_edit(void)
{
    memset(&s_mtp.active_edit, 0, sizeof(s_mtp.active_edit));
}

static void mtp_clear_partial_write(void)
{
    memset(&s_mtp.partial_write, 0, sizeof(s_mtp.partial_write));
}

static void mtp_clear_active_buffer(void)
{
    free(s_mtp.active_buffer.data);
    memset(&s_mtp.active_buffer, 0, sizeof(s_mtp.active_buffer));
}

static void mtp_clear_pending_prop_set(void)
{
    s_mtp.pending_prop_set_active = false;
    s_mtp.pending_prop_set_response = 0;
}

static void mtp_clear_deferred_response(void)
{
    s_mtp.deferred_response_active = false;
    s_mtp.deferred_response_op = 0;
    s_mtp.deferred_response_code = 0;
}
...
static void mtp_clear_objects_for_storage(const struct tinyusb_mtp_storage_s *storage)
{
    for (size_t i = 0; i < CONFIG_TINYUSB_MTP_MAX_OBJECTS; i++) {
        if (s_mtp.objects[i].used && s_mtp.objects[i].storage == storage) {
            mtp_free_object(&s_mtp.objects[i]);
        }
    }
}
...
static bool mtp_utf8_to_mtp_string_payload(const char *value, uint8_t *payload, size_t payload_size, uint32_t *payload_len)
{
...
static int32_t mtp_update_object_stat_locked(mtp_object_t *object)
{
    struct stat st;
...
static int32_t mtp_get_active_edit_object_locked(uint32_t handle, mtp_object_t **object)
{
    if (!s_mtp.active_edit.active || s_mtp.active_edit.handle != handle) {
...
static int32_t mtp_begin_edit_object_locked(uint32_t handle)
{
    ...
    if (s_mtp.active_edit.active && s_mtp.active_edit.handle != handle) {
        ESP_LOGW(TAG, "MTP edit already active for handle %" PRIu32, s_mtp.active_edit.handle);
    ...
    s_mtp.active_edit.active = true;
    s_mtp.active_edit.handle = handle;
    MTP_TRACEI("MTP edit begin: handle=%" PRIu32 " path=%s size=%" PRIu64, handle, object->path, object->size);
    return MTP_RESP_OK;
}

static int32_t mtp_write_object_range_locked(mtp_object_t *object, uint64_t offset, const uint8_t *data, size_t len)
{
    if (len == 0) {
        return MTP_RESP_OK;
    }
...
static int32_t mtp_truncate_object_locked(mtp_object_t *object, uint64_t length)
{
...
static struct tinyusb_mtp_storage_s *mtp_storage_from_id(uint32_t storage_id)
{
    for (size_t i = 0; i < CONFIG_TINYUSB_MTP_MAX_STORAGES; i++) {
        if (s_mtp.storages[i].used && s_mtp.storages[i].storage_id == storage_id) {
            return &s_mtp.storages[i];
        }
    }
    return NULL;
}
...
static mtp_object_t *mtp_object_from_handle(uint32_t handle)
{
    ...
    for (size_t i = 0; i < CONFIG_TINYUSB_MTP_MAX_OBJECTS; i++) {
        if (s_mtp.objects[i].used && s_mtp.objects[i].handle == handle) {
            return &s_mtp.objects[i];
        }
    }
    return NULL;
}
...
{
    uint32_t handle = s_mtp.pending_write.handle;
    const char *path = s_mtp.pending_write.path;
    const char *created_path = s_mtp.pending_write.write_path ? s_mtp.pending_write.write_path : path;

    if (handle == MTP_OBJECT_HANDLE_INVALID && path == NULL && s_mtp.pending_write.file == NULL) {
    ...
    if (s_mtp.pending_write.file) {
        fclose(s_mtp.pending_write.file);
        s_mtp.pending_write.file = NULL;
    }
    if (created_path && s_mtp.pending_write.created_path) {
        mtp_remove_created_path(created_path, s_mtp.pending_write.directory);
    }
    if (handle != MTP_OBJECT_HANDLE_INVALID && s_mtp.pending_write.created_object) {
        mtp_object_t *object = mtp_object_from_handle(handle);
        if (object) {
            mtp_free_object(object);
        }
    }
    s_mtp.pending_write.error_response = error_response;
    mtp_clear_pending_write();
}
...
{
    *storage = NULL;
    if (parent_handle == 0 || parent_handle == MTP_ROOT_PARENT) {
        return 0;
    }
...
{
    for (size_t i = 0; i < CONFIG_TINYUSB_MTP_MAX_OBJECTS; i++) {
        if (s_mtp.objects[i].used && s_mtp.objects[i].storage == storage && strcmp(s_mtp.objects[i].path, path) == 0) {
            return &s_mtp.objects[i];
        }
    }
    return NULL;
...
static uint32_t mtp_next_handle(void)
{
    for (uint32_t tries = 0; tries < UINT32_MAX - 1U; tries++) {
        uint32_t handle = s_mtp.next_object_handle++;
        if (s_mtp.next_object_handle == MTP_OBJECT_HANDLE_INVALID || s_mtp.next_object_handle == MTP_ROOT_PARENT) {
...
static mtp_object_t *mtp_get_or_create_object(struct tinyusb_mtp_storage_s *storage, uint32_t parent, const char *path, const struct stat *st)
{
    ...
    for (size_t i = 0; i < CONFIG_TINYUSB_MTP_MAX_OBJECTS; i++) {
        if (!s_mtp.objects[i].used) {
            object = &s_mtp.objects[i];
                                 ...
                                 size_t max_handles, uint32_t *count)
{
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (object_format == 0 || object_format == 0xFFFFFFFFU || (uint16_t)object_format == fmt) {
            mtp_object_t *object = mtp_get_or_create_object(storage, parent_handle == MTP_ROOT_PARENT ? 0 : parent_handle, path, &st);
            if (object == NULL) {
                free(path);
                closedir(dir);
                return MTP_RESP_STORE_FULL;
...
{
    for (size_t i = 0; i < CONFIG_TINYUSB_MTP_MAX_OBJECTS; i++) {
        if (s_mtp.objects[i].used && s_mtp.objects[i].storage == storage && mtp_path_is_child_of(s_mtp.objects[i].path, path)) {
            mtp_free_object(&s_mtp.objects[i]);
        }
    }
}
...
{
    for (size_t i = 0; i < CONFIG_TINYUSB_MTP_MAX_OBJECTS; i++) {
        if (s_mtp.objects[i].used && &s_mtp.objects[i] != keep && s_mtp.objects[i].storage == storage && mtp_path_is_child_of(s_mtp.objects[i].path, path)) {
            mtp_free_object(&s_mtp.objects[i]);
        }
    }
}
...
{
    if (!s_mtp.deferred_response_active) {
        s_mtp.deferred_response_active = true;
        s_mtp.deferred_response_op = op_code;
        s_mtp.deferred_response_code = response_code;
    }
}

static int32_t mtp_take_deferred_response_locked(uint16_t op_code, int32_t default_response)
{
    if (!s_mtp.deferred_response_active || s_mtp.deferred_response_op != op_code) {
        return default_response;
    }
    int32_t response_code = s_mtp.deferred_response_code;
...
{
    const char *target_path = s_mtp.pending_write.path;
    const char *write_path = s_mtp.pending_write.write_path;
    const char *backup_path = s_mtp.pending_write.backup_path;
    ...
    if (rename(write_path, target_path) != 0) {
        ...
        s_mtp.pending_write.created_path = false;
        return MTP_RESP_GENERAL_ERROR;
    }
...
static int32_t mtp_start_buffered_data(tud_mtp_cb_data_t *cb_data, uint16_t op_code, uint8_t *data, uint32_t len)
{
    ...
    s_mtp.active_buffer.active = true;
    s_mtp.active_buffer.op_code = op_code;
    s_mtp.active_buffer.data = data;
    s_mtp.active_buffer.len = len;
...
static int32_t mtp_continue_buffered_data(tud_mtp_cb_data_t *cb_data)
{
    const mtp_container_command_t *command = cb_data->command_container;
    mtp_lock();
    if (!s_mtp.active_buffer.active || s_mtp.active_buffer.op_code != command->header.code || s_mtp.active_buffer.data == NULL) {
        mtp_unlock();
        return MTP_RESP_GENERAL_ERROR;
    }
    ...
    if (offset >= s_mtp.active_buffer.len) {
        mtp_unlock();
        return 0;
    }

    uint32_t chunk = s_mtp.active_buffer.len - offset;
    if (chunk > container->payload_bytes) {
        chunk = container->payload_bytes;
    }
    memcpy(container->payload, s_mtp.active_buffer.data + offset, chunk);
    if (!tud_mtp_data_send(container)) {
        mtp_clear_active_buffer();
        mtp_unlock();
        return MTP_RESP_DEVICE_BUSY;
...
esp_err_t tinyusb_mtp_install_driver(const tinyusb_mtp_driver_config_t *config)
{
    ...
    s_mtp.version = mtp_strdup_or_default(config ? config->version : NULL, MTP_DEFAULT_VERSION);
    s_mtp.serial = mtp_strdup_or_default(config ? config->serial : NULL, MTP_DEFAULT_SERIAL);
...
esp_err_t tinyusb_mtp_uninstall_driver(void)
{
    ...
    for (size_t i = 0; i < CONFIG_TINYUSB_MTP_MAX_STORAGES; i++) {
        free(s_mtp.storages[i].base_path);
...
esp_err_t tinyusb_mtp_register_storage(const tinyusb_mtp_storage_config_t *config, tinyusb_mtp_storage_handle_t *handle)
{
    ...
    mtp_lock();
    for (size_t i = 0; i < CONFIG_TINYUSB_MTP_MAX_STORAGES; i++) {
        if (s_mtp.storages[i].used && strcmp(s_mtp.storages[i].base_path, config->base_path) == 0) {
            mtp_unlock();
            return ESP_ERR_INVALID_STATE;
        }
    }
    ...
    for (; index < CONFIG_TINYUSB_MTP_MAX_STORAGES; index++) {
        if (!s_mtp.storages[index].used) {
            storage = &s_mtp.storages[index];
            break;
        }
    }
    if (storage == NULL) {
...
esp_err_t tinyusb_mtp_unregister_storage(tinyusb_mtp_storage_handle_t handle)
{
    ...
    if (s_mtp.pending_write.storage_id == handle->storage_id) {
        mtp_abort_pending_write_locked(MTP_RESP_STORE_NOT_AVAILABLE);
    }
    if (s_mtp.active_read.storage == handle) {
        mtp_clear_active_read();
    }
    mtp_clear_active_edit();
    mtp_clear_partial_write();
...
int32_t tud_mtp_data_complete_cb(tud_mtp_cb_data_t *cb_data)
{
    ...
    case MTP_OP_SEND_OBJECT_INFO:
        ...
        if (cb_data->xfer_result != XFER_RESULT_SUCCESS) {
            mtp_clear_deferred_response();
            mtp_abort_pending_write_locked(MTP_RESP_GENERAL_ERROR);
            response->header->code = MTP_RESP_GENERAL_ERROR;
            mtp_unlock();
            break;
        }
        int32_t send_info_resp = mtp_take_deferred_response_locked(MTP_OP_SEND_OBJECT_INFO, MTP_RESP_OK);
        if (send_info_resp != MTP_RESP_OK) {
            mtp_abort_pending_write_locked(send_info_resp);
            response->header->code = (uint16_t)send_info_resp;
            mtp_unlock();
            break;
        }
        if (s_mtp.pending_write.handle == MTP_OBJECT_HANDLE_INVALID) {
            response->header->code = MTP_RESP_GENERAL_ERROR;
            mtp_unlock();
            break;
        }
        bool expects_object_data = s_mtp.pending_write.active;
        (void)mtp_container_add_uint32(response, s_mtp.pending_write.storage_id);
        (void)mtp_container_add_uint32(response, s_mtp.pending_write.parent_handle);
        (void)mtp_container_add_uint32(response, s_mtp.pending_write.handle);
        response->header->code = MTP_RESP_OK;
    ...
    case MTP_OP_SEND_OBJECT:
        ...
        int32_t send_object_resp = mtp_take_deferred_response_locked(MTP_OP_SEND_OBJECT, MTP_RESP_OK);
        if (send_object_resp != MTP_RESP_OK) {
            response->header->code = (uint16_t)send_object_resp;
            mtp_unlock();
            break;
        }
        if (!s_mtp.pending_write.active || s_mtp.pending_write.handle == MTP_OBJECT_HANDLE_INVALID || s_mtp.pending_write.path == NULL) {
            response->header->code = MTP_RESP_GENERAL_ERROR;
            mtp_unlock();
            break;
        }
        ...
        if (s_mtp.pending_write.written != s_mtp.pending_write.expected_size) {
            ESP_LOGE(TAG, "incomplete MTP object write: expected=%" PRIu64 " written=%" PRIu64,
                     s_mtp.pending_write.expected_size, s_mtp.pending_write.written);
            mtp_abort_pending_write_locked(MTP_RESP_INCOMPLETE_TRANSFER);
            response->header->code = MTP_RESP_INCOMPLETE_TRANSFER;
            mtp_unlock();
            break;
        }
        if (s_mtp.pending_write.replace_existing) {
            int32_t replace_resp = mtp_commit_replacement_locked();
            if (replace_resp != MTP_RESP_OK) {
                response->header->code = (uint16_t)replace_resp;
                if (s_mtp.pending_write.path != NULL) {
                    mtp_clear_pending_write();
                }
                mtp_unlock();
                break;
        ...
        if (s_mtp.pending_write.path) {
            struct stat st;
            if (stat(s_mtp.pending_write.path, &st) == 0) {
                mtp_object_t *object = mtp_object_from_handle(s_mtp.pending_write.handle);
                if (object) {
                    mtp_update_object_from_stat(object, &st);
                }
            } else {
                ESP_LOGW(TAG, "failed to stat written MTP object %s: %s", s_mtp.pending_write.path, strerror(errno));
            }
        }
        response->header->code = MTP_RESP_OK;
        MTP_TRACEI("MTP object write complete: handle=%" PRIu32 " bytes=%" PRIu64 " replace=%d path=%s",
                   s_mtp.pending_write.handle, s_mtp.pending_write.written, s_mtp.pending_write.replace_existing, s_mtp.pending_write.path);
        mtp_clear_pending_write();
        mtp_unlock();
        break;
    ...
    case MTP_OP_SET_OBJECT_PROP_VALUE:
        mtp_lock();
        int32_t prop_resp = mtp_take_deferred_response_locked(MTP_OP_SET_OBJECT_PROP_VALUE, 0);
        if (cb_data->xfer_result != XFER_RESULT_SUCCESS) {
            mtp_clear_deferred_response();
            response->header->code = MTP_RESP_GENERAL_ERROR;
        } else if (prop_resp > MTP_RESP_UNDEFINED) {
            response->header->code = (uint16_t)prop_resp;
        } else {
            response->header->code = s_mtp.pending_prop_set_active ? (uint16_t)s_mtp.pending_prop_set_response : MTP_RESP_GENERAL_ERROR;
        }
        mtp_clear_pending_prop_set();
        mtp_unlock();
        break;
    ...
    case MTP_OP_ANDROID_SEND_PARTIAL_OBJECT:
        mtp_lock();
        int32_t partial_resp = mtp_take_deferred_response_locked(MTP_OP_ANDROID_SEND_PARTIAL_OBJECT, MTP_RESP_OK);
        if (cb_data->xfer_result != XFER_RESULT_SUCCESS) {
            mtp_clear_deferred_response();
            response->header->code = MTP_RESP_GENERAL_ERROR;
        } else if (partial_resp != MTP_RESP_OK) {
            response->header->code = (uint16_t)partial_resp;
        } else if (!s_mtp.partial_write.active || s_mtp.partial_write.written != s_mtp.partial_write.expected_size) {
            ESP_LOGE(TAG, "incomplete MTP partial write: expected=%" PRIu32 " written=%" PRIu32,
                     s_mtp.partial_write.expected_size, s_mtp.partial_write.written);
            response->header->code = MTP_RESP_INCOMPLETE_TRANSFER;
        } else {
            response->header->code = MTP_RESP_OK;
        }
    ...
    case MTP_OP_GET_OBJECT:
        mtp_lock();
        if (s_mtp.active_read.active) {
            MTP_TRACEI("MTP object read complete: handle=%" PRIu32 " sent=%" PRIu64 "/%" PRIu64 " result=%d path=%s",
                       s_mtp.active_read.handle, s_mtp.active_read.sent, s_mtp.active_read.expected, cb_data->xfer_result,
                       s_mtp.active_read.path ? s_mtp.active_read.path : "(unknown)");
        } else {
            ESP_LOGW(TAG, "MTP object read complete without active state");
        }
        mtp_clear_active_read();
        mtp_unlock();
        response->header->code = (cb_data->xfer_result == XFER_RESULT_SUCCESS) ? MTP_RESP_OK : MTP_RESP_GENERAL_ERROR;
        break;

    case MTP_OP_GET_PARTIAL_OBJECT:
    case MTP_OP_ANDROID_GET_PARTIAL_OBJECT_64: {
        uint32_t sent = cb_data->total_xferred_bytes > sizeof(mtp_container_header_t) ? cb_data->total_xferred_bytes - sizeof(mtp_container_header_t) : 0;
        mtp_lock();
        if (s_mtp.active_read.active) {
            sent = s_mtp.active_read.sent > UINT32_MAX ? UINT32_MAX : (uint32_t)s_mtp.active_read.sent;
            MTP_TRACEI("MTP partial object read complete: handle=%" PRIu32 " sent=%" PRIu64 "/%" PRIu64 " result=%d path=%s",
                       s_mtp.active_read.handle, s_mtp.active_read.sent, s_mtp.active_read.expected, cb_data->xfer_result,
                       s_mtp.active_read.path ? s_mtp.active_read.path : "(unknown)");
        } else {
    ...
    mtp_trace_request_result(cb_data, response->header->code);
...
static int32_t mtp_get_device_info(tud_mtp_cb_data_t *cb_data)
{
    mtp_container_info_t *container = &cb_data->io_container;
    mtp_lock();
    (void)mtp_container_add_utf8_string(container, s_mtp.manufacturer);
    (void)mtp_container_add_utf8_string(container, s_mtp.model);
    (void)mtp_container_add_utf8_string(container, s_mtp.version);
    (void)mtp_container_add_utf8_string(container, s_mtp.serial);
    mtp_unlock();
...
static int32_t mtp_get_storage_ids(tud_mtp_cb_data_t *cb_data)
{
    ...
    mtp_lock();
    for (size_t i = 0; i < CONFIG_TINYUSB_MTP_MAX_STORAGES; i++) {
        if (s_mtp.storages[i].used) {
            ids[count++] = s_mtp.storages[i].storage_id;
        }
    }
    mtp_unlock();
    ...
    if (storage_id == MTP_ROOT_PARENT) {
        ...
        if (parent_storage) {
            ...
            for (size_t i = 0; i < CONFIG_TINYUSB_MTP_MAX_STORAGES; i++) {
                ...
                uint32_t count = 0;
                int32_t ret = mtp_scan_children(&s_mtp.storages[i], parent, object_format, NULL, 0, &count);
                if (ret > MTP_RESP_UNDEFINED) {
                    mtp_unlock();
                    return ret;
                }
    ...
    if (storage_id == MTP_ROOT_PARENT) {
        ...
        if (parent_storage) {
            ...
            for (size_t i = 0; i < CONFIG_TINYUSB_MTP_MAX_STORAGES; i++) {
                ...
                int32_t ret = mtp_scan_children(&s_mtp.storages[i], parent, object_format, handles + count, CONFIG_TINYUSB_MTP_MAX_OBJECTS - count, &sub_count);
                if (ret > MTP_RESP_UNDEFINED) {
                    mtp_unlock();
                    free(handles);
                    return ret;
...
static int32_t mtp_start_read(tud_mtp_cb_data_t *cb_data, uint64_t offset, uint64_t length)
{
    ...
    mtp_clear_active_read();
    s_mtp.active_read.active = true;
    s_mtp.active_read.file = file;
    s_mtp.active_read.storage = storage;
    s_mtp.active_read.handle = handle;
    s_mtp.active_read.expected = length;
    s_mtp.active_read.remaining = length;
    s_mtp.active_read.sent = 0;
    s_mtp.active_read.path = path;
    path = NULL;
    ...
    if (first_len > 0 && fread(container->payload, 1, first_len, file) != first_len) {
        ESP_LOGE(TAG, "failed to read first MTP object chunk from %s", s_mtp.active_read.path);
        mtp_clear_active_read();
        mtp_unlock();
        return MTP_RESP_GENERAL_ERROR;
    }
    s_mtp.active_read.sent = first_len;
    s_mtp.active_read.remaining -= first_len;
    ...
    MTP_TRACEI("MTP object read begin: handle=%" PRIu32 " offset=%" PRIu64 " bytes=%" PRIu64 " object_size=%" PRIu64 " first=%u path=%s",
               handle, offset, length, object_size, (unsigned)first_len, s_mtp.active_read.path);
    int32_t resp = tud_mtp_data_send(container) ? 0 : MTP_RESP_DEVICE_BUSY;
    if (resp != 0) {
        mtp_clear_active_read();
    }
...
{
    mtp_lock();
    if (!s_mtp.active_read.active || s_mtp.active_read.file == NULL) {
    ...
    mtp_container_info_t *container = &cb_data->io_container;
    size_t chunk = s_mtp.active_read.remaining < container->payload_bytes ? (size_t)s_mtp.active_read.remaining : container->payload_bytes;
    if (chunk == 0) {
        mtp_unlock();
        return 0;
    }
    if (fread(container->payload, 1, chunk, s_mtp.active_read.file) != chunk) {
        ESP_LOGE(TAG, "failed to read MTP object chunk from %s", s_mtp.active_read.path ? s_mtp.active_read.path : "(unknown)");
        mtp_clear_active_read();
        mtp_unlock();
        return MTP_RESP_GENERAL_ERROR;
    }
    s_mtp.active_read.sent += chunk;
    s_mtp.active_read.remaining -= chunk;
    MTP_TRACEI("MTP object read chunk: handle=%" PRIu32 " chunk=%u sent=%" PRIu64 "/%" PRIu64 " remaining=%" PRIu64 " path=%s",
               s_mtp.active_read.handle, (unsigned)chunk, s_mtp.active_read.sent, s_mtp.active_read.expected, s_mtp.active_read.remaining,
               s_mtp.active_read.path ? s_mtp.active_read.path : "(unknown)");
    int32_t resp = tud_mtp_data_send(container) ? 0 : MTP_RESP_DEVICE_BUSY;
    if (resp != 0) {
        mtp_clear_active_read();
...
static int32_t mtp_send_partial_object(tud_mtp_cb_data_t *cb_data)
{
    ...
    if (cb_data->phase == MTP_PHASE_COMMAND) {
        ...
        if (ret == MTP_RESP_OK) {
            s_mtp.partial_write.active = true;
            s_mtp.partial_write.handle = handle;
            s_mtp.partial_write.offset = offset;
            s_mtp.partial_write.expected_size = data_len;
            s_mtp.partial_write.written = 0;
    ...
    mtp_lock();
    if (!s_mtp.partial_write.active || s_mtp.partial_write.handle != handle) {
        mtp_unlock();
        return MTP_RESP_INVALID_OBJECT_HANDLE;
    }
    ...
    if (ret == MTP_RESP_OK) {
        uint32_t remaining = s_mtp.partial_write.expected_size - s_mtp.partial_write.written;
        size_t to_write = container->payload_bytes < remaining ? container->payload_bytes : remaining;
        ret = mtp_write_object_range_locked(object, s_mtp.partial_write.offset + s_mtp.partial_write.written, container->payload, to_write);
        if (ret == MTP_RESP_OK) {
            s_mtp.partial_write.written += (uint32_t)to_write;
            if (s_mtp.partial_write.written < s_mtp.partial_write.expected_size) {
...
static int32_t mtp_send_object_info(tud_mtp_cb_data_t *cb_data)
{
    ...
    s_mtp.pending_write.handle = object->handle;
    s_mtp.pending_write.storage_id = storage->storage_id;
    s_mtp.pending_write.parent_handle = parent_handle == MTP_ROOT_PARENT ? 0 : parent_handle;
    s_mtp.pending_write.expected_size = info->object_compressed_size;
    s_mtp.pending_write.path = strdup(path);
    s_mtp.pending_write.write_path = write_path;
    s_mtp.pending_write.backup_path = backup_path;
    s_mtp.pending_write.active = !is_dir;
    s_mtp.pending_write.created_path = created_path;
    s_mtp.pending_write.created_object = created_object;
    s_mtp.pending_write.replace_existing = replace_existing;
    s_mtp.pending_write.directory = is_dir;
    if (s_mtp.pending_write.path == NULL) {
    ...
    MTP_TRACEI("MTP object info write: handle=%" PRIu32 " storage=0x%08" PRIx32 " parent=%" PRIu32 " size=%" PRIu64 " dir=%d replace=%d path=%s",
               object->handle, storage->storage_id, s_mtp.pending_write.parent_handle, s_mtp.pending_write.expected_size, is_dir, replace_existing, path);
    free(path);
    mtp_unlock();
    return 0;
}
...
static int32_t mtp_send_object(tud_mtp_cb_data_t *cb_data)
{
    ...
    mtp_lock();
    if (!s_mtp.pending_write.active || s_mtp.pending_write.path == NULL) {
        mtp_unlock();
        return MTP_RESP_INVALID_OBJECT_HANDLE;
    }

    if (cb_data->phase == MTP_PHASE_COMMAND) {
        container->header->len = sizeof(mtp_container_header_t) + (uint32_t)s_mtp.pending_write.expected_size;
        ...
        if (resp != 0) {
            ...
            MTP_TRACEI("MTP object write begin: handle=%" PRIu32 " bytes=%" PRIu64 " path=%s",
                       s_mtp.pending_write.handle, s_mtp.pending_write.expected_size, s_mtp.pending_write.path);
        }
        mtp_unlock();
        return resp;
    }

    if (s_mtp.pending_write.file == NULL) {
        const char *write_path = s_mtp.pending_write.write_path ? s_mtp.pending_write.write_path : s_mtp.pending_write.path;
        s_mtp.pending_write.file = fopen(write_path, "wb");
        if (s_mtp.pending_write.file == NULL) {
    ...
    uint64_t remaining = s_mtp.pending_write.expected_size - s_mtp.pending_write.written;
    ...
    if (to_write > 0) {
        if (fwrite(container->payload, 1, to_write, s_mtp.pending_write.file) != to_write) {
            const char *write_path = s_mtp.pending_write.write_path ? s_mtp.pending_write.write_path : s_mtp.pending_write.path;
        ...
        s_mtp.pending_write.written += to_write;
    }
                                                           ...
                                                           uint32_t depth)
{
    ...
    if (ret != MTP_RESP_OK || !object->directory || depth == 0) {
        return ret;
    }

    uint32_t *handles = calloc(CONFIG_TINYUSB_MTP_MAX_OBJECTS, sizeof(uint32_t));
    if (handles == NULL) {
        ESP_LOGE(TAG, "failed to allocate MTP prop list child handle buffer");
        return MTP_RESP_STORE_FULL;
    }

    uint32_t count = 0;
    ret = mtp_scan_children(object->storage, object->handle, 0, handles, CONFIG_TINYUSB_MTP_MAX_OBJECTS, &count);
    if (ret == MTP_RESP_OK) {
        uint32_t child_depth = depth == MTP_ROOT_PARENT ? MTP_ROOT_PARENT : depth - 1U;
        for (uint32_t i = 0; i < count; i++) {
            mtp_object_t *child = mtp_object_from_handle(handles[i]);
            if (child == NULL) {
                continue;
            }
            ret = mtp_builder_append_object_tree_props_locked(builder, child, object_format, prop_code, child_depth);
            if (ret != MTP_RESP_OK) {
                break;
            }
        }
...
static int32_t mtp_get_object_prop_list(tud_mtp_cb_data_t *cb_data)
{
    ...
    if (object_handle == MTP_ROOT_PARENT) {
        for (size_t i = 0; i < CONFIG_TINYUSB_MTP_MAX_OBJECTS; i++) {
            ...
            ret = mtp_builder_append_object_props_locked(&builder, &s_mtp.objects[i], object_format, prop_code);
            if (ret != MTP_RESP_OK) {
                break;
            }
        }
...
static int32_t mtp_get_device_property(tud_mtp_cb_data_t *cb_data)
{
    ...
    if (command->header.code == MTP_OP_GET_DEVICE_PROP_DESC) {
        ...
        (void)mtp_container_add_utf8_string(container, s_mtp.friendly_name);
        (void)mtp_container_add_utf8_string(container, s_mtp.friendly_name);
        (void)mtp_container_add_uint8(container, 0);
    } else {
        (void)mtp_container_add_utf8_string(container, s_mtp.friendly_name);
...
{
    ESP_RETURN_ON_FALSE(storage && storage->used && path && object_handle, ESP_ERR_INVALID_ARG, TAG, "invalid test find args");
    struct stat st;
    ESP_RETURN_ON_FALSE(stat(path, &st) == 0, ESP_ERR_NOT_FOUND, TAG, "test object path missing: %s", path);

    mtp_lock();
    mtp_object_t *object = mtp_get_or_create_object(storage, 0, path, &st);
    if (object == NULL) {
        mtp_unlock();
        return ESP_ERR_NO_MEM;
    }
...
esp_err_t tinyusb_mtp_test_end_edit_object(uint32_t object_handle)
{
    ...
    mtp_clear_partial_write();
    if (s_mtp.active_edit.active && s_mtp.active_edit.handle == object_handle) {
        mtp_clear_active_edit();
    }
    mtp_unlock();
    return ret == MTP_RESP_OK ? ESP_OK : ESP_FAIL;
