...
static int esp_jpeg_decoder_one_picture(uint8_t *input_buf, int len, uint8_t *output_buf)
{
    ...
    if (jpeg_io == NULL) {
        ESP_LOGE(TAG, "jpeg_io malloc failed");
        return ESP_FAIL;
    }
    ...
    if (out_info == NULL) {
        ESP_LOGE(TAG, "out_info malloc failed");
        return ESP_FAIL;
    }
...
int jpeg_quick_check(const uint8_t *data, size_t len)
{
    if (len < 10) {
        return -1;
    }
...
{
    // printf("camera_frame_cb frame size: %ld x %ld, data_bytes: %d\n", frame->width, frame->height, frame->data_bytes);
    if (jpeg_quick_check((const uint8_t *)frame->data, frame->data_bytes) != 0) {
        ESP_LOGW(TAG, "Dropped invalid jpeg frame");
        return;
    }
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(app_uvc_control_dev_by_resolution(0, 800, 480, 1));
#else
