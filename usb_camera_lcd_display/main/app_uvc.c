...
static void camera_cb_task(void *arg);
...
void print_jpg(uint8_t *data, int len)
{
    printf(">>>JPEG data (%d bytes):\n", len);
    for (int i = 0; i < len; i++) {
        printf("%02X ", data[i]);
        if ((i + 1) % 48 == 0) {
            printf("\n");
...
static void camera_cb_task(void *arg)
{
    ...
    for (;;) {
        uvc_host_frame_t *host_frame = NULL;
        host_frame = app_uvc_get_frame_by_index(dev->index);
        if (!host_frame) {
            vTaskDelay(1);
            continue;
        }
        uvc_frame_t frame_data = {
            .width = host_frame->vs_format.h_res,
            .height = host_frame->vs_format.v_res,
            .data_bytes = host_frame->data_len,
            .data = host_frame->data,
        };
        camera_frame_cb(&frame_data, NULL);
        app_uvc_return_frame_by_index(dev->index, host_frame);
    }
}
...
bool frame_callback(const uvc_host_frame_t *frame, void *user_ctx)
{
    ...
    case UVC_VS_FORMAT_MJPEG:
        ESP_LOGD(TAG, "Cam[%d] Frame received, size: %d", dev->index, frame->data_len);
        break;
    default:
...
{
    BaseType_t task_created = xTaskCreatePinnedToCore(usb_lib_task, "usb_lib", 4096, xTaskGetCurrentTaskHandle(), 15, NULL, 0);
    assert(task_created == pdPASS);
...
esp_err_t app_uvc_control_dev_by_resolution(int dev_index, int width, int height, bool if_open)
{
    uvc_dev_info_t dev_info;
    app_uvc_get_dev_frame_info(dev_index, &dev_info);
    for (int i = 0; i < dev_info.resolution_count; i++) {
        if (dev_info.resolution[i].width == width && dev_info.resolution[i].height == height) {
            return app_uvc_control_dev_by_index(dev_index, if_open, i);
        }
    }
    return ESP_ERR_NOT_FOUND;
}
