...
static esp_err_t fusb302_measure_status(fusb302_dev_t *dev, typec_cc_status_t *st)
{
    ...
    /* Sink base: Rd on both CCs, clear MEAS, no Rp/VCONN */
    ESP_RETURN_ON_ERROR(i2c_rd8(d, REG_SWITCHES0, &sw0), TAG, "rd SW0");
    sw0 &= ~(SW0_CC1_PU_EN | SW0_CC2_PU_EN | SW0_VCONN_CC1 | SW0_VCONN_CC2 | SW0_MEAS_CC2);
    sw0 |=  (SW0_CC1_PD_EN | SW0_CC2_PD_EN |  SW0_MEAS_CC1);
    ESP_RETURN_ON_ERROR(i2c_wr8(d, REG_SWITCHES0, sw0), TAG, "meas CC1");
    ...
    ESP_RETURN_ON_ERROR(i2c_wr8(d, REG_SWITCHES0, sw0), TAG, "meas CC2");
    esp_rom_delay_us(100);
    ...
    st->rp_cur_ma  = (bclvl == ST0_BC_LVL_200_600)  ? 500  :
                     ...
                     (bclvl == ST0_BC_LVL_1230_MAX) ? 3000 : 0;

    ESP_LOGI(TAG, "CC: att=%d act=%s CC1=0x%02X CC2=0x%02X Rp=%dmA VBUSOK=%d",
             st->attached, st->cc2_active ? "CC2" : "CC1",
             bclvl_cc1, bclvl_cc2, st->rp_cur_ma, vbus_ok);
    return ESP_OK;
}
...
esp_err_t fusb302_set_role(fusb302_dev_t *dev, esp_typec_power_role_t role)
{
    ...
    case ESP_TYPEC_PWR_SOURCE: {  // Configure as Source (DFP)
        ...
        value = SW1_POWERROLE | CTL0_HOST_CUR_MEDIUM;              // POWERROLE=1 (source), 180µA Rp for 1.5A
        ...
        value |= CTL0_HOST_CUR_MEDIUM;                          // HOST_CUR for 1.5A
        ESP_RETURN_ON_ERROR(i2c_wr8(d, REG_CONTROL0, value), TAG, "set HOST_CUR");
        // (If needed, handle VCONN enable here if supporting powered cables)
        ESP_LOGI(TAG, "FUSB302 set as Source");
        break;
    }
...
esp_err_t fusb302_service_irq(fusb302_dev_t *dev,
                              ...
                              typec_cc_status_t *st_out)
{
    if (!dev || !events || !st_out) {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    *events = 0;
    ...
    // Always return a fresh snapshot at the end
