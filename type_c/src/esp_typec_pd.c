...
static inline void pd_set_vbus_source(pd_port_ctx_t *ctx, bool enable)
{
    if (ctx->cfg.src_vbus_gpio == GPIO_NUM_NC) {
        return; // no MOSFET controlled by PD stack
    }
    gpio_set_level(ctx->cfg.src_vbus_gpio, enable);
}
...
static void pd_task(void *arg)
{
    ...
    for (;;) {
        ...
        ctx->backend->service_irq(ctx->ctrl, &ev);
        ...
        if (gpio_get_level(ctx->gpio_int) == 0) {
            ...
            (void)ctx->backend->service_irq(ctx->ctrl, &ev);
        }
                                           ...
                                           esp_typec_pd_port_handle_t *port_hdl_ret)
{
    ...
    ESP_RETURN_ON_FALSE(port_cfg->rp_current <= ESP_TYPEC_RP_3A0,
                        ESP_ERR_INVALID_ARG, TAG, "rp_current invalid");
    ...
    // Configure VBUS MOSFET GPIO if present
    if (ctx->cfg.src_vbus_gpio != GPIO_NUM_NC) {
        gpio_config_t io = {
    ...
    ESP_RETURN_ON_ERROR(fusb302_set_rp_current(dev, port_cfg->rp_current), TAG, "set Rp current failed");
    ctx->ctrl = dev;
                                      ...
                                      esp_typec_power_role_t role)
{
    ...
    // Simple safety policy: only SOURCE is allowed to drive VBUS
