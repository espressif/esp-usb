...
static const char *TAG = "example_typec_fusb302";
...
/* ===== Event callback ===== */
static void on_typec_event(void *handler_arg, esp_event_base_t base, int32_t id, void *event_data)
{
    (void)handler_arg;
    (void)base;
    ...
    case ESP_TYPEC_EVENT_ID_ATTACHED: {
        ...
        ESP_LOGI(TAG, "ATTACHED: cc2=%d, rp=%u mA", cc2, (unsigned)ma);
        break;
    }
...
void app_main(void)
{
    ...
    /* (Optional change power role) */
    //ESP_ERROR_CHECK(esp_typec_set_power_role(port, ESP_TYPEC_PWR_SOURCE));
