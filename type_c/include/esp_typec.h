...
/**
 * @brief Type-C library info structure.
 */
typedef struct {
...
/**
 * @brief Uninstall and deinitialize the Type-C library.
 * @return ESP_OK on success
 */
esp_err_t esp_typec_uninstall(void);

/**
 * @brief Get Type-C library info (number of ports, etc).
...
/**
 * @brief Hardware configuration for HUSB320 (CC-only, no PD).
 */
typedef struct {
...
/**
 * @brief Create a Type‑C CC-only port using HUSB320 (no PD).
 * @param port_cfg Port configuration
 * @param hw_cfg Hardware configuration for HUSB320
...
/**
 * @brief Power-role control (CC Rp/Rd).
